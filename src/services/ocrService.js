...
export function extractStructuredMrvData(text, baseConfidence = 85) {
  ...
  const projectMatch = text.match(/(?:Project\s*(?:Code|ID|Number)?[:\s-]+|PRJ-)([A-Z0-9-]+)/i);
  if (projectMatch) {
    structured.projectId = projectMatch[1].startsWith('PRJ-') ? projectMatch[1] : (projectMatch[0].includes('PRJ') ? projectMatch[0].replace(/[^A-Z0-9-]/gi, '') : projectMatch[1]);
    extractedFieldCount++;
  }
  ...
  const refMatch = text.match(/(?:Reference|Audit|Challan|Certificate|Order|Assay)\s*(?:No|Number|#)?[:\s-]+([A-Z0-9\-_]+)/i);
  if (refMatch) {
