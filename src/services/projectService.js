...
export async function createProject(formData) {
  ...
  const metadata = {
    treeDensity: Number(formData.treeDensity) || 1800,
    targetPlants: Number(formData.targetPlants) || 260000,
    species: formData.species || 'Avicennia marina, Rhizophora mucronata',
    socBaseline: Number(formData.socBaseline) || 2.8,
    communityName: formData.communityName || 'Local Coastal Community',
    communityContact: formData.communityContact || '',
    revenueShare: Number(formData.revenueShare) || 35,
    localJobs: Number(formData.localJobs) || 120,
  ...
  const payload = {
    ...
    name: formData.name || 'Untitled Blue Carbon Project',
    organization_id: organizationId,
    type: formData.type || 'Mangrove Restoration',
    location: formData.location || 'Ratnagiri, Maharashtra',
    state: formData.state || 'Maharashtra',
    country: formData.country || 'India',
    latitude: formData.lat || formData.latitude || 16.9902,
    longitude: formData.lng || formData.longitude || 73.3120,
    area: Number(formData.area) || 0,
    est_co2e: Number(formData.estCO2e) || Number(formData.est_co2e) || 0,
